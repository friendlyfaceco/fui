import type {SliderProps as RACSliderProps, SliderRenderProps} from 'react-aria-components';
import {Slider as RACSlider, SliderThumb, SliderTrack as RACSliderTrack} from 'react-aria-components';
    ...
    return (
        <RACSliderTrack className="group relative flex w-full items-center orientation-horizontal:h-7 orientation-vertical:h-44 orientation-vertical:w-7">
            {({state, orientation}) => {
                return (
                    <>
                        <div className={twMerge('bg-zinc-200 dark:bg-zinc-300', trackStyle)} />
                        <div
...
function getTrackHighlightStyle(state: SliderRenderProps['state'], orientation: SliderRenderProps['orientation']) {
