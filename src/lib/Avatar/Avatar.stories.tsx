...
AvatarSizes.parameters = {
    docs: {
        description: {
            story: 'Use `className="size-*"` to render avatars with different sizes. Default avatar size is `size-10`.',
        },
    },
};
...
InitialFallbacks.parameters = {
    docs: {
        description: {
            story: 'The `alt` prop is used to generate initial avatar when image is loading or src is unavailable. Use the `colorless` prop to render initial avatars with a single background.',
        },
    },
};
...
AvatarBadges.parameters = {
    docs: {
        description: {
            story: 'Use the `AvatarBadge` component to add badges. Badge is auto wrapped within the <a href="./?path=/docs/icon--docs" target="_blank">`Icon`</a> component and auto scaled based on avatar sizes.\n\nUse `aria-label` to provide accessible badge. Use `className` to style badge border and background.',
        },
    },
};
