...
import {LabelContext, type Key} from 'react-aria-components';
import type {ListData} from 'react-stately';
import {twMerge} from 'tailwind-merge';

import {Tag, TagGroup, TagList} from '../TagGroup';
import {Input, TextField} from '../TextField/TextField';
...
export function TagsInputField({list, name, onTagRemove, onTagAdd, ...props}: TagInputProps) {
    return (
        <TagInputContext.Provider value={{list, onTagAdd, onTagRemove}}>
            <TextField {...props} />
            {name && <input name={name} hidden readOnly value={list.items.map(({name}) => name).join(',')} />}
        </TagInputContext.Provider>
    );
}

export function TagInput({className}: {className?: string; children?: React.ReactNode}) {
        ...
        <TagGroup
            ...
            <div
                className={twMerge(
                    ...
                    'has-[input[data-focused=true]]:ring-1 has-[input[data-focused=true]]:ring-blue-500 dark:text-white'
                )}>
