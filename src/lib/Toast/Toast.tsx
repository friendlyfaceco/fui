...
import type {ToastConfig} from './ToastQueue';
...
function ToastRegion({state, ...props}: ToastRegionProps) {
    ...
    const position = state.visibleToasts[state.visibleToasts.length - 1].content.position ?? 'bottom-right';
    ...
    return (
        <div
            {...regionProps}
            ref={ref}
            className={twMerge('toast-region fixed isolate z-20 flex flex-col gap-2 outline-none', className)}>
...
export function Toast({state, ...props}: ToastProps) {
    ...
    switch (position) {
        ...
        case 'top-right':
            enteringClassName =
                props.toast.animation === 'entering' ? 'duration-200 slide-in-from-right animate-in ease-out' : '';
            break;
        case 'bottom-left':
        case 'top-left':
            enteringClassName =
                props.toast.animation === 'entering' ? 'duration-200 slide-in-from-left animate-in ease-out' : '';
            break;
        case 'bottom-center':
        case 'top-center':
            enteringClassName =
                props.toast.animation === 'entering' ? 'duration-200 slide-in-from-top animate-in ease-out' : '';
            break;
        default:
            break;
    }
    ...
    return (
        <div {...toastProps} ref={ref} className="flex flex-1 rounded-lg bg-background outline-none">
            <div
                className={twMerge(
                    'toast flex w-[min(85vw,360px)] gap-1 rounded-lg border px-3 py-2 shadow-sm transition',
                    type === undefined && !props.toast.content.render && 'border-border/50 bg-background',
                        ...
                        <div className="flex flex-1 items-center gap-2 self-center">
                            ...
                            <div className="flex flex-1 flex-col gap-1 text-sm/6">
                                {props.toast.content.title ? (
                                    <div
                                        {...titleProps}
                                        className={twMerge(props.toast.content.description && 'font-medium')}>
                                        {props.toast.content.title}
                                    </div>
                                ) : null}
