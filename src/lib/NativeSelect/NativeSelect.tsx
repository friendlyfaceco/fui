...
import {selectBoxIndicator, inputFieldStyle, focusVisibleRingStyle} from '../utils';
import {DescriptionContext, DescriptionProvider} from '../TextField/TextField';

export function NativeSelectField({className, ...props}: JSX.IntrinsicElements['div']) {
        ...
        <LabelContext.Provider value={{id: labelId, elementType: 'span'}}>
            <DescriptionProvider>
                <div
                    ...
                    className={twMerge('has-[select:disabled]:opacity-50', inputFieldStyle, className)}
                />
            </DescriptionProvider>
...
export function NativeSelect({
    ...
    return (
        <div data-ui="control" className={twMerge('relative flex transition', selectBoxIndicator)}>
            <select
                {...focusProps}
