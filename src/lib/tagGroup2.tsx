...
import {Tag as AriaTag, TagGroup as AriaTagGroup, Button, TagList as RACTagList} from 'react-aria-components';
...
const colors = {
    default: ['selected:border-accent', 'selected:bg-accent', 'selected:text-white', 'selected:outline-0'],
    success: [
        'bg-success/10',
        'border-success/15',
        'dark:bg-success/35',
...
export function TagList<T extends object>(props: TagListProps<T>) {
    return <RACTagList {...props} className={composeTailwindRenderProps(props.className, 'flex flex-wrap gap-1')} />;
}
