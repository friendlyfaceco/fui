...
import type {TabListProps, TabPanelProps, TabProps, TabsProps as RACTabProps} from 'react-aria-components';
import {Tab as RACTab, TabList as RACTabList, TabPanel as RACTabPanel, Tabs as RACTabs} from 'react-aria-components';
...
export type TabsProps = RACTabProps & ({variant?: 'underline' | 'pills'} | {variant: 'segment'; orientation?: never});
...
export function TabList<T extends object & {title: string}>(props: TabListProps<T>) {
...
const tab = {
    underline: {
        ...
        horizontal: ['before:bottom-[-1.5px]', 'before:w-full', 'before:inset-x-0', 'selected:before:h-[2px]', 'p-2'],
        vertical: [
            'before:inset-y-0',
    ...
    pills: {
        base: ['flex', 'items-center', 'px-3', 'py-2', 'rounded-lg', 'selected:bg-accent', 'selected:text-white'],
        horizontal: [],
        vertical: [],
    },
    segment: {
