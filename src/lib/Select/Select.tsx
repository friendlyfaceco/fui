...
import {selectBoxIndicator, composeTailwindRenderProps, inputFieldStyle, focusVisibleRingStyle} from '../utils';
...
export function SelectListItem({destructive, ...props}: ListBoxItemProps & {destructive?: true}) {
...
export function SelectListItemLabel({className, ...props}: JSX.IntrinsicElements['span']) {
    return <span {...props} slot="label" data-ui="label" className={twMerge('mb-0 truncate w-full', className)} />;
}

export function SelectListItemDescription({className, ...props}: JSX.IntrinsicElements['span']) {
