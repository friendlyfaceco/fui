...
export function Small({className, ...props}: JSX.IntrinsicElements['small']) {
    return <Text {...props} elementType="small" className={twMerge('text-sm sm:text-xs', className)} />;
}

export function TextLink(props: LinkProps) {
    return <Link {...props} className={composeTailwindRenderProps(props.className, 'underline underline-offset-4')} />;
}
